...
9. **NEVER delete `.sandstone/` in `sandstone-template/`.** Not `.sandstone/output/`, not `.sandstone/cache.json`, not the whole folder. The template's CLI uses `.sandstone/cache.json` to do incremental builds — it hashes the fully-visited-and-serialized resource data in-memory and only rewrites files whose bytes actually changed. If a file looks stale after a build, the generator deliberately produced the same bytes; do not "fix" it by clearing the cache. To rebuild after editing source, just re-run `bun dev:build` from the template directory. Only delete `.sandstone/` when the user explicitly tells you to do so. See `sandstone-template/CLAUDE.md` for the full cache explanation.
10. **Once the workspace is known to be linked (`bun dev:link` has run, packages resolve via `node_modules/sandstone` etc.), trust the builder — do NOT look at `dist/`, `lib/`, or `node_modules/<pkg>/dist` to investigate output.** The builder is doing its job. Inspect generated output in the consumer project's actual output directory (e.g. `sandstone-template/.sandstone/output/` for the template, `.sandstone/` outputs in other consumers) — that's the truth. If a generator's output looks wrong, fix the SOURCE in `sandstone/src/` and re-run the consumer's build. Only dig into the sandstone library builder (`sandstone/scripts/`, `sandstone/scripts/plugins/`) when the user explicitly tells you to.
